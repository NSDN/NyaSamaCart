 ...
 */
public class CartRenderer extends RenderMinecart {

    private ResourceLocation texture;
    private ModelBase model;

    public CartRenderer(ModelBase model) {
        super();
        this.model = model;
        this.texture = null;
    }
    ...
    protected ResourceLocation getEntityTexture(EntityMinecart cart) {
        if (cart instanceof INyaSamaCart) {
            if (((INyaSamaCart) cart).getData(INyaSamaCart.CART_TEXT) == null) {
                texture = new ResourceLocation("nsc", "textures/carts/default.png");
            } else {
                texture = new ResourceLocation("nsc", "textures/carts/" + ((INyaSamaCart) cart).getData(INyaSamaCart.CART_TEXT) + ".png");
            }
        }
        return texture;
    }
    ...
    public void doRender(EntityMinecart minecart, double x, double y, double z, float Yaw, float p_doRender_9_) {
        ...
        GL11.glPushMatrix();
        GL11.glTranslatef(0.0F, -1.03125F, 0.0F);
        GL11.glRotatef(90.0F, 0.0F, 1.0F, 0.0F);
        model.render(minecart, 0.0F, 0.0F, -0.1F, 0.0F, 0.0F, 0.0625F);
        GL11.glPopMatrix();
